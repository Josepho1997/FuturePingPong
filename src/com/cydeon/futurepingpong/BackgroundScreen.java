...
public class BackgroundScreen extends GLScreen{
	SpriteBatcher batcher;
	Camera2D guiCam;
	Random ran;
	Float ranR, ranG, ranB, ranA;
	String bgC;
	Vector2 touchPoint;
    int x = 0;
	...
	public BackgroundScreen(Game game) {
		...
		ranR = Settings.rgbBackground[0];
		ranG = Settings.rgbBackground[1];
		ranB = Settings.rgbBackground[2];
	    ...
	    for(int i = 0; i < len; i++) {
	        ...
	        if(event.type == TouchEvent.TOUCH_UP) {
	            ...
	            if(OverlapTester.pointInRectangle(grcBounds, touchPoint)) {
	        		...
	        		ranB = ran.nextFloat();
	        		ranA = ran.nextFloat();
	            	Settings.rgbBackground[0] = ranR;
	  		        Settings.rgbBackground[1] = ranG;
	  		        Settings.rgbBackground[2] = ranB;
	...
	public void present(float deltaTime) {
	        ...
	        gl.glPushMatrix();
	        batcher.drawSprite((720 / 3) / 2, 768 - ((96 / 3) / 2), 720 / 3, 96 / 3, Assets.backgroundColor);
	        ...
	        //Red RGB Background
	        String bRanRP = Integer.toString(Math.round(ranR * 100));
	        String bRanRFRGB = Integer.toString(Math.round(ranR * 256));
	        Assets.font.drawText(batcher,  "R: " + bRanRP + "%" + "(" + bRanRFRGB + ")", 1280 - /*((bRanRP.length() + bRanRFRGB.length() + 6) * 16)*/ 192, 748 - 40);
	        
	        //Green RGB Backgorund
	        String bRanGP = Integer.toString(Math.round(ranG * 100));
	        String bRanGFRGB = Integer.toString(Math.round(ranG * 256));
	        Assets.font.drawText(batcher,  "G: " + bRanGP + "%" + "(" + bRanGFRGB + ")", 1280 - /*((bRanGP.length() + bRanGFRGB.length() + 6) * 16)*/ 192, 708 - 20);
	        
	        //Blue RGB Backgorund
	        String bRanBP = Integer.toString(Math.round(ranB * 100));
	        String bRanBFRGB = Integer.toString(Math.round(ranB * 256));
