...
public class PauseScreen extends GLScreen{
	...
	public PauseScreen(Game game) {
		...
		pC = "Pause Button's Current Color: ";
	    ...
	    for(int i = 0; i < len; i++) {
	        ...
	        if(event.type == TouchEvent.TOUCH_UP) {
	            ...
	            if(OverlapTester.pointInRectangle(grcBounds, touchPoint)) {
	        		...
	        		ranB = ran.nextFloat();
	        		ranA = ran.nextFloat();
	            	Settings.rgbPause[0] = ranR;
	  		        Settings.rgbPause[1] = ranG;
	  		        Settings.rgbPause[2] = ranB;
	...
	public void present(float deltaTime) {
	        ...
	        gl.glPushMatrix();
	        batcher.drawSprite((468 / 3) / 2, 768 - ((84 / 3) / 2), 468 / 3, 84 / 3, Assets.pauseColor);
	        ...
	        gl.glPushMatrix();
	        gl.glColor4f(Settings.rgbPause[0], Settings.rgbPause[1], Settings.rgbPause[2], 1.0f);
//	        /Log.d("Red", Float.toString(Settings.rgbPaddle1[0]));
	        batcher.drawSprite((1280 / 2) - (74 / 2), 768 / 2, 27, 180, Assets.paddle);
	        batcher.drawSprite((1280 / 2) + (74 / 2), 768 / 2, 27, 180, Assets.paddle);
	        gl.glPopMatrix();
	        batcher.endBatch();
	        ...
	        Assets.font.drawText(batcher, pC, 1280 - (16 * (pC.length() - 1)), 600);
	        batcher.endBatch();
	        
	        batcher.beginBatch(Assets.key);
	        //Red RGB Paddle
	        String ranRgbR = Integer.toString(Math.round(Settings.rgbPause[0] * 100));
	        String ranBRRGB = Integer.toString(Math.round(Settings.rgbPause[0] * 256));
	        Assets.font.drawText(batcher,  "R: " + ranRgbR + "%" + "(" + ranBRRGB + ")", 1280 - /*((ranRgbR.length() + ranBRRGB.length() + 6) * 16)*/ 192, 600 - 40);

	        gl.glColor4f(1, 1, 1, 1);
	        //Green RGB Paddle
	        String ranRgbG = Integer.toString(Math.round(Settings.rgbPause[1] * 100));
	        String ranBGRGB = Integer.toString(Math.round(Settings.rgbPause[1] * 256));
	       // Log.d("Test", ranRgbG + " " + ranBGRGB);
	        ...
	        //Blue RGB Paddle
	        String ranRgbB = Integer.toString(Math.round(Settings.rgbPause[2] * 100));
	        String ranBBRGB = Integer.toString(Math.round(Settings.rgbPause[2] * 256));
